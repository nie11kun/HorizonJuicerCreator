...
bool FileWork::findAndReplaceInString(string &in, const char *find,
                                      const char *replace) {
  boost::regex e;
  e.assign(find);
  std::ostringstream t(std::ios::out | std::ios::binary);
  std::ostream_iterator<char, char> oi(t);
  boost::regex_replace(oi, in.begin(), in.end(), e, replace,
...
bool FileWork::findAndRepleaceInFile(string filename, const char *findMe,
  ...
  try {
    ...
    boost::regex_replace(out, in.begin(), in.end(), e, replaceMe,
                         boost::match_default | boost::format_all);
    os.close();
...
bool FileWork::findAndReplaceFromJSONWithIgnore(const char *dir,
                                                const char *jsonData,
                                                const char *ignore[],
                                                int ignoreCount) {
  // 解析JSON并创建替换映射
    ...
    for (fs::directory_iterator dir_itr(p); dir_itr != end_iter; ++dir_itr) {
      try {
        if (fs::is_directory(dir_itr->status())) {
          // 如果需要可以处理子目录
          this->findAndReplaceFromJSONWithIgnore(
              dir_itr->path().string().c_str(), jsonData, ignore, ignoreCount);
        } else if (fs::is_regular_file(dir_itr->status())) {
          if (ignore == NULL ||
              searchWithKey(dir_itr->path().filename().string(), ignore,
                            ignoreCount) == 0) {
            // 使用所有替换处理文件
            findAndReplaceMultipleInFile(dir_itr->path().string(),
                                         replacements);
          } else {
            cout << "******ignored file: "
...
bool FileWork::findAndReplaceMultipleInFile(
...
bool FileWork::findAndRepleaceInDirWithIgnore(const char *dir,
                                              const char *findMe,
                                              const char *replaceMe,
                                              const char *ignore[],
                                              int ignoreCount) {
  ...
  if (fs::is_directory(p)) {
    ...
    for (fs::directory_iterator dir_itr(p); dir_itr != end_iter; ++dir_itr) {
      try {
        if (fs::is_directory(dir_itr->status())) {
          // this->findAndRepleaceInDir(dirNow, findMe, replaceMe, fileEx,
          // iCount);
        } else if (fs::is_regular_file(dir_itr->status())) {
          if (ignore == NULL ||
              searchWithKey(dir_itr->path().filename().string(), ignore,
...
bool FileWork::findAndRepleaceInDirWithInclude(const char *dir,
                                               const char *findMe,
                                               const char *replaceMe,
                                               const char *include[],
                                               int includeCount) {
    ...
    for (fs::directory_iterator dir_itr(p); dir_itr != end_iter; ++dir_itr) {
      try {
        if (fs::is_directory(dir_itr->status())) {
          ...
          if (include == NULL ||
              searchWithKey(dir_itr->path().filename().string(), include,
                            includeCount) == 1) {
            findAndRepleaceInFile(dir_itr->path().string(), findMe, replaceMe);
          } else {
            cout << "******ignored file: "
...
bool FileWork::findAndRepleaceInDirWithIgnoreRecursion(const char *dir,
                                                       const char *findMe,
                                                       const char *replaceMe,
                                                       const char *ignore[],
                                                       int ignoreCount) {
    ...
    for (fs::directory_iterator dir_itr(p); dir_itr != end_iter; ++dir_itr) {
      try {
        if (fs::is_directory(dir_itr->status())) {
          ...
          this->findAndRepleaceInDirWithIgnoreRecursion(
              c_dirNow, findMe, replaceMe, ignore, ignoreCount);
        } else if (fs::is_regular_file(dir_itr->status())) {
          if (ignore == NULL ||
              searchWithKey(dir_itr->path().filename().string(), ignore,
                            ignoreCount) == 0) {
...
bool FileWork::findAndRepleaceInDirWithIncludeRecursion(const char *dir,
                                                        const char *findMe,
                                                        const char *replaceMe,
                                                        const char *include[],
                                                        int includeCount) {
    ...
    for (fs::directory_iterator dir_itr(p); dir_itr != end_iter; ++dir_itr) {
      try {
        if (fs::is_directory(dir_itr->status())) {
          string dirNow = dir_itr->path().string();
          const char *c_dirNow = dirNow.c_str();
          this->findAndRepleaceInDirWithIncludeRecursion(
              c_dirNow, findMe, replaceMe, include, includeCount);
        } else if (fs::is_regular_file(dir_itr->status())) {
          if (include == NULL ||
              searchWithKey(dir_itr->path().filename().string(), include,
                            includeCount) == 1) {
...
bool FileWork::copyFilesToNewDirWithIgnore(const char *oldDir,
                                           const char *newDir,
                                           const char *ignore[],
                                           int ignoreCount) {
    ...
    for (fs::directory_iterator dir_itr(p); dir_itr != end_iter; ++dir_itr) {
      try {
        if (fs::is_directory(dir_itr->status())) {
          ...
          if (ignore == NULL || searchWithKey(dir_itr->path().string(), ignore,
                                              ignoreCount) == 0) {

            fs::path newDic = np;
            newDic += pathSeparator + dir_itr->path().filename().string();

            if (fs::is_regular_file(newDic)) {
              fs::remove(newDic);
            }
            fs::copy_file(*dir_itr, newDic);
            std::cout << "copied file: " << dir_itr->path().filename().string()
                      << endl;
          } else {
            cout << "******ignored file: "
                 << dir_itr->path().filename().string() << endl;
...
bool FileWork::copyFilesToNewDirWithInclude(const char *oldDir,
                                            const char *newDir,
                                            const char *include[],
                                            int includeCount) {
  fs::path p = fs::system_complete(oldDir);
    ...
    for (fs::directory_iterator dir_itr(p); dir_itr != end_iter; ++dir_itr) {
      try {
        if (fs::is_directory(dir_itr->status())) {
          ...
          if (include == NULL || searchWithKey(dir_itr->path().string(),
...
bool FileWork::copyFilesToNewDirWithIncludeDir(
    const char *oldDir, const char *newDir, const char *includeDir[],
    int includeDirCount, const char *includeFiles[], int includeFilesCount) {
  fs::path p = fs::system_complete(oldDir);
  fs::path np = fs::system_complete(newDir);
  ...
  if (fs::is_directory(p)) {
    ...
    for (fs::directory_iterator dir_itr(p); dir_itr != end_iter; ++dir_itr) {
      try {
        if (fs::is_directory(dir_itr->status())) {
          if (includeDir == NULL ||
              searchWithKey(dir_itr->path().string(), includeDir,
                            includeDirCount) == 1) {
            string dirNow = dir_itr->path().string();
            string slng;
            if (lngIn == 0) {
              slng = pathSeparator + "chs";
            } else {
              slng = pathSeparator + "eng";
            }
            dirNow = dirNow + slng;
            const char *c_dirNow = dirNow.c_str();
            this->copyFilesToNewDirWithIncludeDir(c_dirNow, newDir, includeDir,
                                                  includeDirCount, includeFiles,
                                                  includeFilesCount);
          }
        } else if (fs::is_regular_file(dir_itr->status())) {
          if (includeFiles == NULL ||
              searchWithKey(dir_itr->path().string(), includeFiles,
                            includeFilesCount) == 1) {
            fs::path newDic = np;
            newDic += pathSeparator + dir_itr->path().filename().string();
...
bool FileWork::copyFilesToNewDirWithRefDir(const char *oldDir,
                                           const char *newDir,
                                           const char *refDir,
                                           const char *include[],
                                           int includeCount) {
      ...
      try {
        if (fs::is_directory(dir_itr->status())) {
          ...
          if (searchWithKeyAndRefDir(refDir, dirNow)) {
            ...
            copyFilesToNewDirWithInclude(oldDIrChs, newDir, include,
                                         includeCount);
          }
        } else if (fs::is_regular_file(dir_itr->status())) {
          fs::path newDic = np;
...
bool FileWork::copyFilesToNewDirWithIgnoreRefDirs(
    const char *oldDir, const char *refDir[], int refDirCount,
    const char *newDir, const char *ignore[], int ignoreCount) {
  fs::path p = fs::system_complete(oldDir);
  fs::path np = fs::system_complete(newDir);
  ...
  if (fs::is_directory(p)) {
    ...
    for (fs::directory_iterator dir_itr(p); dir_itr != end_iter; ++dir_itr) {
      try {
        if (fs::is_directory(dir_itr->status())) {
          if (refDir != NULL && searchWithKey(dir_itr->path().string(), refDir,
                                              refDirCount) == 1) {
            string dirNow = dir_itr->path().string();
            const char *c_dirNow = dirNow.c_str();
            this->copyFilesToNewDirWithIgnore(c_dirNow, newDir, ignore,
                                              ignoreCount);
          }
        } else if (fs::is_regular_file(dir_itr->status())) {
          if (ignore == NULL || searchWithKey(dir_itr->path().string(), ignore,
...
bool FileWork::copyFilesToNewDirWithIgnoreRecursion(const char *oldDir,
                                                    const char *newDir,
                                                    const char *ignore[],
                                                    int ignoreCount) {
  fs::path p = fs::system_complete(oldDir);
    ...
    for (fs::directory_iterator dir_itr(p); dir_itr != end_iter; ++dir_itr) {
      try {
        if (fs::is_directory(dir_itr->status())) {
          ...
          s_childDir += pathSeparator + dir_itr->path().filename().string();
          const char *c_childDir = s_childDir.c_str();

          string s_p = oldDir;
          s_p += pathSeparator + dir_itr->path().filename().string();
...
void FileWork::load_file(std::string &s, std::istream &is) {
  ...
  // 检查 tellg 是否成功（返回-1表示失败）
  if (endPos == std::streampos(-1) || !is.good()) {
    // 回退到使用 stringstream 读取
    std::stringstream buffer;
    buffer << is.rdbuf();
    s = buffer.str();
    return;
  ...
  std::streamsize size = static_cast<std::streamsize>(endPos);
  ...
  if (size > 0) {
    ...
    is.read(&s[0], size);
    // 调整字符串大小为实际读取的字节数
    s.resize(static_cast<size_t>(is.gcount()));
  }
}
...
int FileWork::searchWithKeyAndRefDir(const char *refDir, string str) {
  rsize_t found = str.find_last_of(pathSeparator);
...
bool FileWork::renameFileInDir(const char *dir, const char *find[],
                               string name) {
  fs::path p = fs::system_complete(dir);
  fs::path np = p;
  np += pathSeparator + name + ".MPF";
    ...
    for (fs::directory_iterator dir_itr(p); dir_itr != end_iter; ++dir_itr) {
      try {
        if (fs::is_directory(dir_itr->status())) {
          ...
          if (find != NULL &&
              searchWithKey(dir_itr->path().string(), find, 1) == 1) {
            string dirNow = dir_itr->path().string();
      ...
      try {
        if (fs::is_directory(dir_itr->status())) {
          ...
          if (include == NULL ||
              searchWithKey(dir_itr->path().filename().string(), include,
                            includeCount) == 1) {
            if (!resizeImageFile(dir_itr->path().string(), targetWidth,
                                 targetHeight)) {
              std::cout << "resizing error: "
                        << dir_itr->path().filename().string() << std::endl;
            }
          } else {
            std::cout << "******ignored: "
                      << dir_itr->path().filename().string() << std::endl;
